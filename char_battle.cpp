...
bool CHARACTER::Attack(LPCHARACTER pkVictim, BYTE bType)
{
	...
	// ✅ NOVÉ: Lag-compensated distance check (jen pro PC vs PC)
	if (IsPC() && pkVictim->IsPC())
	{
		// Použij výchozí lag 150ms (průměrný ping)
		// Pokud chcete dynamické ping detection, musíte ho implementovat v DESC class
		DWORD dwAverageLag = 150;  // 150ms výchozí lag compensation

		// Získej historickou pozici oběti
		...
		// Spočítej vzdálenost s lag compensation
		float fDistance = DISTANCE_SQRT(
			(GetX() - lVictimX) / 100.0f,
			(GetY() - lVictimY) / 100.0f
		);

		// Získej attack range podle zbraně
		float fAttackRange = 170.0f;  // Default melee range

		if (GetWear(WEAR_WEAPON))
		{
			LPITEM pkWeapon = GetWear(WEAR_WEAPON);
			BYTE bWeaponSubType = pkWeapon->GetSubType();
			
			switch (bWeaponSubType)
			{
				case WEAPON_SWORD:      // Jednoruční meč
					fAttackRange = 170.0f;
					break;

				case WEAPON_DAGGER:     // Dýka
					fAttackRange = 140.0f;
					break;					

				case WEAPON_TWO_HANDED: // Obouruční zbraň
					fAttackRange = 200.0f;  // Delší dosah pro obouručku
					break;
					
				case WEAPON_BOW:        // Luk
					fAttackRange = 1000.0f;
					break;
					
				case WEAPON_BELL:       // Zvon (Shaman)
				case WEAPON_FAN:        // Vějíř (Shaman)
					fAttackRange = 500.0f;  // Magic weapons
					break;
		...
		{
			if (test_server)
				sys_log(0, "Attack MISS: %s -> %s distance %.2f > range %.2f (lag comp %dms)",
					GetName(), pkVictim->GetName(), fDistance, fAttackRange, dwAverageLag);

			// Prostě vrať false - útok neproběhne
			// Klient si zobrazí miss animaci automaticky
			return false;
		}

        if (test_server)
            sys_log(0, "Attack HIT: %s -> %s distance %.2f <= range %.2f (lag comp %dms)",
                GetName(), pkVictim->GetName(), fDistance, fAttackRange, dwAverageLag);
    }
